...
> **Release candidate status:** `3.0.0-rc.2` is the current source candidate for a Trusted Publisher release. Until its Registry and provenance checks are recorded, `3.0.0-rc.1` remains the published baseline verified below. Use `@next` for evaluation and pin an exact reviewed version for deployment.
...
`3.0.0-rc.2` changes release metadata and documentation only. Its Registry,
provenance, and clean-install checks are not claimed before publication. The
runtime baseline remains the following `3.0.0-rc.1` evidence.
