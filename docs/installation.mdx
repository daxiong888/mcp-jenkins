...
> **Note:** Pass credentials via `MCP_JENKINS_*` environment variables or your client's `env` config — not `--api-token` / `--bearer-token` CLI arguments, which are visible in process lists and shell history. The examples below follow this rule.
