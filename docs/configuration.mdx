...
**All credentials via environment (recommended):**

```bash
MCP_JENKINS_URL=https://jenkins.com MCP_JENKINS_USER=admin MCP_JENKINS_API_TOKEN=abc123 node dist/index.js
```

**Mixed (CLI overrides env):**

```bash
MCP_JENKINS_USER=dev_user MCP_JENKINS_API_TOKEN=xyz789 node dist/index.js --url https://jenkins.com
# Uses: URL from CLI, credentials from env
```

> **Note:** `--api-token` / `--bearer-token` CLI flags still work, but command-line secrets are visible in process lists and shell history — pass credentials via environment variables or your MCP client's `env` config instead.

## 🔐 Security Best Practices

- ✅ Use environment variables (never hardcode tokens)
- ✅ Keep credentials out of CLI arguments — they leak into process lists and shell history
