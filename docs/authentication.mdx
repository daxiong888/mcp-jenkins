...
**Bearer Token (Recommended):** set `MCP_JENKINS_BEARER_TOKEN` in your MCP client's `env` config.

**Basic Auth:** set `MCP_JENKINS_USER` and `MCP_JENKINS_API_TOKEN` in your MCP client's `env` config.

Inject either via the client config or a secret manager — never on an interactive command line, where token values land in shell history.
