...
> **Release candidate status:** the independent npm package is not published yet. Until it is available in the registry, use the local installation option below.
...
For `3.0.0-rc.1`, the current source passed `npm test` (23 files, 244 tests),
`npm run typecheck`, and `npm run build` locally on macOS arm64 with Node.js
v24.11.1. An `npm pack` output from the same HEAD was installed in a disposable
directory and completed an MCP stdio `initialize` handshake. This verifies the
package candidate can be built and installed locally; it does not verify npm
...
GitHub Actions is configured for Node.js 20, 22, and 24, but those hosted jobs
have not run for the current unpushed commits; only Node.js v24.11.1 is locally
verified. No write request was sent to a real Jenkins instance, and the local
controllers do not reproduce real plugin sets, security policies, SSO, or
