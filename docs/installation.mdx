...
> **Release candidate status:** `@daxiong888/mcp-jenkins@3.0.0-rc.2` is published through npm Trusted Publishing with provenance. Use `@next` for evaluation and pin `@3.0.0-rc.2` for a reviewed deployment.
...
For `3.0.0-rc.2`, release metadata validation, 9 focused manifest/version
tests, `npm test` (23 files, 244 tests), `npm run typecheck`, `npm run build`,
and the npm publish dry-run passed locally on macOS arm64 with Node.js v24.11.1.
The published `@next` tarball contains 199 files (51.8 kB) and matched npm's
...
covered write paths. The matrix was not rerun for the metadata-only rc.2
candidate. Replay is unit/mock tested but excluded from Docker because the stock
controller images lack the required Pipeline/Workflow plugins.

[GitHub Actions run 33134322828](https://github.com/daxiong888/mcp-jenkins/actions/runs/33134322828)
